    ...
    <div class="profile-wrapper">
      ...
      <div v-else>
        <div class="text-center">
          ...
          <h2>{{ user.name }}</h2>
          <button class="btn" @click="logoutMe">Выйти</button>
        </div>

        <div v-if="user.role === 'client'">
          <h3>История заказов</h3>
          <div class="order" v-for="order in getOrderByIdArr(user.ordersHistory)" :key="order.id">
            ...
            <p>
              ...
              <span v-for="(prod, idx) in getProductsArrByIDArr(order.products)" :key="idx">
        ...
        <div v-else>
          ...
          <div class="order" v-for="order in activeOrder" :key="order.id">
            <div class="order-header">
              <h4>Номер заказа {{ order.id }}</h4>
            ...
            <button class="btn status status-return" @click="changeOrderStatus(order.id, 'canceled')"></button>
            <button class="btn status status-done" @click="changeOrderStatus(order.id, 'done')"></button>
          </div>
          <h3>История заказов</h3>
          <div class="order" v-for="order in inactiveOrder" :key="order.id">
            <div class="order-header">
              <h4>Номер заказа {{ order.id }}</h4>
              <div class="status" :class="{
                'status-return': order.status == 'canceled',
