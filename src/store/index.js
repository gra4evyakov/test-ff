...
export default createStore({
  ...
  getters: {
    ...
    totalCartPrice: (state) => {
      if (state.inCart.length) {
        return state.inCart.reduce(
          (total, current) =>
            Number(total) + Number(current.price) * Number(current.count),
          0
        );
      } else {
        return 0;
  ...
  mutations: {
    ...
    REMOVE_EXCLUDED_INGREDIENT(state, params) {
      params[0].excludedIngredients = params[0].excludedIngredients.filter(
        (el) => el != params[1]
      );
    },
    CLEAN_CART(state) {
  ...
  actions: {
    async getProducts({ commit, getters }) {
      try {
        ...
        if (sessionStorage.getItem("inCart")) {
          let inCartSessionStorageArr = sessionStorage
            .getItem("inCart")
            .split(",");
          sessionStorage.removeItem("inCart");
    ...
    loginSubmit({ state, commit }, params) {
      let auth = state.userList.find(
        (user) => user.login == params.login && user.password == params.password
      );
      if (auth) {
  ...
  const products = [
    {
      ...
      description:
        "Мягкая булочка разогрета в микроволновке, да, но котлета то сделана с любовью. За это мы его и любим.",
      count: 1,
    ...
    },
    {
      ...
      description:
        "Когда ты №1, ты знаешь, что ты №1. И все вокруг знают. Они знают, что ты №1!",
      count: 1,
    ...
    },
    {
      ...
      description:
        "Может содержать следы пороха и свинца (металл такой, не мясо)",
      count: 1,
      canPayForEBalls: false,
      showIngredients: false,
      excludedIngredients: [],
    ...
    },
    {
      ...
      description:
        "Особо опасен, так как вызывает привыкание. Мы бы не советовали привыкать, но уже сами привыкли.",
      count: 1,
      canPayForEBalls: false,
      showIngredients: false,
      excludedIngredients: [],
    ...
    },
    {
      ...
      description:
        "Порадуй свою бабушку. Большой черный босс, ягодный морс и картошечка помогут тебе набрать пару-тройку кило.",
      count: 1,
      canPayForEBalls: false,
      showIngredients: false,
      excludedIngredients: [],
